...
```
   ...
   bots     ───► │           (dir, lay, action) per slot │
                 ...
                 │           Blasts, Pickups, Hen token, │  no Node, no wall-clock,
                 │           Respawn, Clock, Scoring,    │  no delta-time reads
                 │           GameMode                    │
...
- The round clock is an **integer tick counter** counting down from `round_ticks` (120 s × 60 in deathmatch, 300 s × 60 in Hen Grenade), never a float accumulation of `delta`. It is the sole round-ending condition. The value comes from `GameMode` when that resource does not inherit, otherwise from `Balance`.
...
- Kill credit reads the owner off the flame tile that killed the player: someone else's, `+1` to them; your own, `−1` to you. In Hen Grenade that tally is flavour; the round winner is the unique highest `hen_ticks`.
- **Hen Grenade state is two fields on `MatchState` and one on `PlayerState`** (M3.5): `hen_slot` (`-1` or a living holder), `hen_token_tile` (`(-1,-1)` while held), and per-player `hen_ticks`. The token is **not** a `Powerup.Kind` and is **not** on the pickup grids, so a crate cannot spawn a second one and a blast cannot delete the objective. Collection uses the same after-move centre-tile test as a pickup. Initial placement is one `next_index` into a pre-filtered candidate list, Hen-mode-only — deathmatch `create()` spends no extra draws. Drop on death is an ordered search and spends none.
- Effective Hen speed is an override read at movement time, the same pattern as the FAST curse, and it wins over FAST. Bomb placement, Toss and Remote are skipped for `hen_slot`; Kick is not. Collecting the token re-arms that player's Remote bombs, the same helper as death-of-a-Remote-holder, so fuse-less walls cannot be left behind by a hunter who just became prey.
...
- `data/balance/match.tres` and `data/balance/match_hen.tres` (`MatchRules`) — round wins to take the match, the round ceiling, scoreboard duration. Hen Grenade is first-to-1; deathmatch is first-to-2.
- `data/balance/modes/deathmatch.tres` and `data/balance/modes/hen.tres` (`GameMode`) — mode id, crate-density scale, optional round-length and crate-cap overrides, Hen speed, scoring rule. The simulation sees this resource; the lobby writes the selection onto a static `Session`, not a new autoload.
- `data/arenas/*.tres` (`ArenaDef`) — grid size, crate density, spawn and respawn tiles, tileset reference. Hen mode scales `crate_permille` at generation time and does not mutate the file.
...
**The replay stores a *rules* fingerprint, not a balance fingerprint** (M3, extended M3.5). It mixes `Balance`, `PowerupTable` and `GameMode`, because a hen-speed retune changes what a recorded round produces just as surely as a re-weighted drop table does. `MatchRules` stays out of that mix: scoreboard duration is not a sim input. The file format's magic stays `HGR1`: the byte layout and the input encoding are unchanged, and what changed is only what goes into that `u32`. An M3 deathmatch replay therefore reports a rules mismatch, which is correct — the mix grew. Replayed under the deathmatch `GameMode`, its *state* fingerprint must still match; that is how we know FFA did not move.
...
M3.5's Hen and token are the same lesson: one silhouette pass and one token pass
for the whole arena, never a per-player branch that breaks the hunter-rect
batch. The stress scene stays a deathmatch worst case; those two passes are
what `-- --measure` is allowed to grow by.
